...
```java
// ExecutionContext.java - Runtime state container
public class ExecutionContext {
    private final Map<String, Object> variables = new HashMap<>();
    private final Queue<IBlueprintNode> executionQueue = new LinkedList<>();
    private final World world;
    private final ServerPlayerEntity player;

    public void executeFrom(IBlueprintNode startNode) {
        executionQueue.add(startNode);
        while (!executionQueue.isEmpty()) {
            IBlueprintNode node = executionQueue.poll();
            node.execute(this);
        }
    }
}
...
```java
// BlueprintExecutor.java - Main execution orchestrator
public class BlueprintExecutor {
    private static final int MAX_NODES_PER_TICK = 1000;

    public ExecutionResult execute(BlueprintGraph graph, IBlueprintNode eventNode,
                                    World world, Object... eventArgs) {
        ExecutionContext context = new ExecutionContext(world, eventArgs);
        try {
            context.executeFrom(eventNode);
...
```java
// nodes/event/OnBlockUseEventNode.java
public class OnBlockUseEventNode extends EventNode {
    public OnBlockUseEventNode() {
        super("on_block_use", "On Block Use");
        addOutput(PinType.EXECUTION_FLOW, "exec");
        addOutput(PinType.ENTITY, "player");
        addOutput(PinType.BLOCK_POS, "blockPos");
        addOutput(PinType.WORLD_REF, "world");
    }
}
```

```java
// nodes/flow/BranchNode.java
public class BranchNode extends BlueprintNode {
    public BranchNode() {
        super("branch", "Branch");
        addInput(PinType.EXECUTION_FLOW, "exec");
        addInput(PinType.BOOLEAN, "condition");
        addOutput(PinType.EXECUTION_FLOW, "true");
        addOutput(PinType.EXECUTION_FLOW, "false");
    }

    @Override
    public void execute(ExecutionContext context) {
        boolean condition = context.getPinValue("condition", Boolean.class);
        String outputPin = condition ? "true" : "false";
        context.executeConnectedNodes(this, outputPin);
    }
}
```
